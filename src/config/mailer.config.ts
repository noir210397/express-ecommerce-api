...
const transporter = nodemailer.createTransport(process.env.NODE_ENV === "development" ? {
    host: "smtp.gmail.com",
    port: 465,
    secure: true,
    auth: {
...
async function sendOrderSuccessEmail(order: Order) {
    try {
...
async function sendAdminOrderNotification(order: Order) {
    try {
...
export async function sendTestEmail(subject: string, text: string) {
...
export { sendAdminOrderNotification, sendOrderSuccessEmail, }